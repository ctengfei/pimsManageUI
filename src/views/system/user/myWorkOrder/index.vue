<template>
    <div class="app-container">
        ...
        <el-form ref="formSearch" inline v-model="formSearch">
          ...
          <el-form-item prop="repairWorkOrderType" label="工单类型">
            <el-select placeholder="请选择工单类型" clearable v-model="formSearch.repairWorkOrderType">
              <el-option
                v-for="(item,index) in dict.type.repair_work_order_type"
                :key="index"
                :value="item.value"
                :label="item.label"
              ></el-option>
      ...
      <el-dialog title="任务详情" :visible.sync="workDetailShow" width="600px">
        <workdetail ref='workDetail' :rowData='rowData' @workDetailVisibleShow='workDetailVisibleShow' v-if="workDetailShow"></workdetail>
      </el-dialog>
      <el-dialog
        :title="title"
  ...
  //        选择人员名单
      ...
      methods: {
        ...
        //           return {
        //             value: result.dictValue,
        //             name: result.dictLabel
        //           };
        //         });
