    ...
    <el-row :gutter="20">
      ...
      <el-col :xl="20" :lg="18" :sm="17" :xs="24">
        ...
        <div class="photoBox" v-if="tableData&&tableData.length">
          <div class="groups" v-for="item in tableData" :key="item.id">
            <div class="imgTitle">
              <h4 style="cursor: pointer;" @click="toDeviceDraw(item)">{{item.deviceName}}</h4>
              <div class="gifs">
                <img class="" :src="require(`@/assets/images/smartBuild/xf.gif`)" alt="item.type"/>
              </div>
            </div>
            <div class="infomation">
              ...
              <div v-for="(items,index) in item.pointRunDataList" :key="index">
                ...
                <div v-else>
                  <span v-show="items.pointType=='freshAirSystem_run'">
                    <span class="pointName">{{items.pointName}}</span>
                    <span class="bgCond bgSucc">{{items.dataValue == 'true' ? '开机' : '关机'}}</span>
                  </span>
                  <span v-show="items.pointType=='freshAirSystem_fault'">
                    <span class="pointName">{{items.pointName}}</span>
                    <span class="bgCond"
                          :style="{'background-color': items.dataValue !== 'true' ? '#00e984' : '#ff403d' }">{{items.dataValue !== 'true' ? '正常' : '故障'}}</span>
                  </span>
                  <span v-show="items.pointType=='freshAirSystem_temperature'">
                    <span class="pointName">{{items.pointName}}</span>
                    <span class="bgCond bgSucc">{{items.dataValue}}{{'℃'}}</span>
                  </span>
                </div>
              </div>
              <span v-if="item.monitorVos && item.monitorVos.length">
                    ...
                    <span class="bgCond" v-for="(child,index) in item.monitorVos" :key="index">
                      <img class="" style="width: 20px;height: 20px;margin-top:4px" :src="require(`@/assets/images/smartBuild/sxt.png`)" @click="oncamera(child)"/>
                    </span>
                  </span>
              <div class="margin0" style="margin-left:20px;margin-top:10px">
                <span v-for="(items,index) in item.pointRunDataList" :key="index">
                  <span v-if="items.pointType == 'freshAirSystem_contr'">
                    <el-button type="primary" @click="submitForm(item)" :disabled="items.dataValue =='true'  ">启动</el-button>
                    <el-button plain @click="resetForm(item)" :disabled="items.dataValue !== 'true'">关闭</el-button>
                  </span>
                </span>
              </div>
            </div>
...
<script>
  ...
  export default {
    name:"NewTrend",
    components: {sysTree, pagination, OrgizeTree, comCard, ComDescriptions, ComDescriptionsItem},
    data() {
      return {
        ruleForm: {
          pass: '',
        ...
        spanList: [
          {
            type: "icon_xinfengshebeizongshu",
            num: 0,
            name: "设备总数",
             path: require(`@/assets/icons/svgui/icon_xinfengshebeizongshu.svg`),
            
          },
          {
            type: "icon_xinfengshebeion",
            num: 0,
            name: "设备开机",
            path: require(`@/assets/icons/svgui/icon_xinfengshebeion.svg`),
          },
          {
            type: "icon_xinfengshebeiclose",
            num: 0,
            name: "设备关机",
            path: require(`@/assets/icons/svgui/icon_xinfengshebeiclose.svg`),
          },
          {
            type: "icon_xinfengshebeiyichang",
            num: 0,
            name: "设备异常",
            path: require(`@/assets/icons/svgui/icon_xinfengshebeiyichang.svg`),
          },
        ],
        crameShow:false,
        rtspUrl:'',
    ...
    methods: {
      ...
      submitForm(data) {
        ...
        data.pointRunDataList.forEach(item => {
          if (item.pointType == 'freshAirSystem_contr') {
            tagid = item.tagId;
          }
        });
        let obj = {
