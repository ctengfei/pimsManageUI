  ...
  <div class="main" @keyup.esc="escLoading">
    <div class="flex">
      <div class="left-masks">
        ...
        <main>
          <div class="highlight-height-table0 padding-10">
            <template >
              <!-- 左侧按钮 -->
              <div id="vedioBoxs">
                <div class="functionMenu">
                  <div>
                    <span class="video-web-content-title" style="margin-left:20px">{{ $props.equipment.deviceName }} 设备 <br/>&nbsp;&nbsp;&nbsp;{{ playTimeYYR }}</span>
                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      permission: {
        ...
        getQueryAlarm: 'monitor::historyVideo::getQueryAlarm',
        getSearchWarning: 'monitor::historyVideo::getSearchWarning',
        getRecordTag: 'monitor::historyVideo::getRecordTag',
        getSearchMasks: 'monitor::historyVideo::getSearchMasks',
        postRecordTag: 'monitor::historyVideo::postRecordTag',
        putRecordTag: 'monitor::historyVideo::putRecordTag',
        deleteRecordTag: 'monitor::historyVideo::deleteRecordTag',
        playerRange: 'monitor::historyVideo::playerRange',
        playerSuspend: 'monitor::historyVideo::playerSuspend',
        playerPlay: 'monitor::historyVideo::playerPlay',
        playerStop: 'monitor::historyVideo::playerStop',
        playerSpeed: 'monitor::historyVideo::playerSpeed',
        postSnapPhoto: 'monitor::historyVideo::postSnapPhoto',
        startRecording: 'monitor::historyVideo::startRecording',
        endRecording: 'monitor::historyVideo::endRecording',
        search:'configure::argumentsConfigure:search',
      },
  ...
  methods: {
      ...
      getIvsPlayer('/sm/ivs/player/play','POST',obj)
        .then(res => {
          if(res.meta.status == 200){
            ...
            this.mySsrc=true
            console.log(temp.startTimestamp,"data.startTimestampdata.startTimestampdata.startTimestamp")
            this.playerStartTime = new Date(temp.startTimestamp).getTime()
            // this.playerNowTimeSlot = {startTime:this.dateValue.time[0],endTime:this.dateValue.time[1]}
          }else{
    ...
    intoPage(sj) {
      ...
      let Vurl = this.$store.getters.btnpremissAll[this.permission.getSearchVideo].url
      let Vmethod = this.$store.getters.btnpremissAll[this.permission.getSearchVideo].method
      // 录像检索
      getSearchVideo(Vurl,Vmethod,Vdata).then(res=>{
        if (res.meta.status === 200) {
...
//      this.searchMasks(false)
      // let Turl = this.$store.getters.btnpremissAll[this.permission.getRecordTag].url
      // let Tmethod = this.$store.getters.btnpremissAll[this.permission.getRecordTag].method
      // getRecordTag(Turl,Tmethod,{deviceId: this.myId.deviceId, timeStamp: this.playTimeYYR + " 00:00:00"}).then(res=>{
      //     if (res.meta.status === 200) {
      //       this.msrksSpan = res.data.data
      //       this.msrksSize = res.data.count
      //     }
      //   }).catch((err) => {
      //     this.$message.error("书签请求失败", err);
      //   });

      // 当前player的id请求 告警标签
      // let Aurl = this.$store.getters.btnpremissAll[this.permission.getQueryAlarm].url
      // let Amethod = this.$store.getters.btnpremissAll[this.permission.getQueryAlarm].method
      // getQueryAlarm(Aurl,Amethod,{ deviceId: this.myId.deviceId, timeStamp: this.playTimeYYR }).then(res=>{
      //     this.warningSize = res.data.count
      //     if (res.meta.status === 200) {
      //       res.data.data.map((item) => {
      //         item.percentage =
      //           (
      //             (new Date(item.alarmTime).getHours() * 3600 +
      //               new Date(item.alarmTime).getMinutes() * 60 +
      //               new Date(item.alarmTime).getSeconds()) /
      //             86400
      //           ).toFixed(3) *
      //             100 +
      //           "%";
      //       });
      //       this.warningSpan =  res.data.data
      //     }
      //   })
      //   .catch((err) => {
      //     this.$message.error("告警请求失败", err);
      //   });
    },
    ...
    async getHistoryVideo(equipment,index,forwardS) {
      ...
      const videoData = {
        ...
        // startTimestamp: new Date(equipment.startTime).getTime() / 1000,
        // endTimestamp: new Date(equipment.endTime).getTime() / 1000,
        startTimestamp: equipment.startTime,
        endTimestamp: equipment.endTime,
      };
      this.goPlayer(videoData)
      // let Hurl = this.$store.getters.btnpremissAll[this.permission.getHistoryVideo].url
      // let Hmethod = this.$store.getters.btnpremissAll[this.permission.getHistoryVideo].method
      // this.playBLoading = true
      // await getHistoryVideo(Hurl.replace('{deviceId}',this.myId.deviceId),Hmethod,videoData)
      // .then((res) => {
      //   this.playBLoading = false
      //   if (res.meta.status === 200) {
      //       this.baseMediaUrl = res.data.mediaIp;
      //       this.mySsrc = res.data.ssrc;
      //       // this.playerUrl = this.getVideoUrlBySsrc(this.mySsrc)
      //       this.playerStartTime = new Date(equipment.startTime).getTime()
      //     }
      //   })
      //   .catch((err) => {
      //     this.playBLoading = false
      ...
      if(forwardS) {
        setTimeout(() => {
          // let Rurl = this.$store.getters.btnpremissAll[this.permission.playerRange].url
          // let Rmethod = this.$store.getters.btnpremissAll[this.permission.playerRange].method
          // playerRange(Rurl.replace('{ssrc}',this.mySsrc).replace('{range}',forwardS),Rmethod).then(res => {
    ...
    playerPlay1() {
      if (this.playButton) {
        // let Surl = this.$store.getters.btnpremissAll[this.permission.playerSuspend].url.replace('{ssrc}',this.mySsrc)
        // let Smethod = this.$store.getters.btnpremissAll[this.permission.playerSuspend].method
        // playerSuspend(Surl,Smethod).then(res => {
        //   
         
        // }).catch(err => {
        //   this.$message.error('暂停请求错误！', err)
        // })
        if(this.TimeInterval) window.clearInterval(this.TimeInterval)
    ...
    // 播放器的 播放
    playerPlay2() {
      //   let Purl = this.$store.getters.btnpremissAll[this.permission.playerPlay].url.replace('{ssrc}',this.mySsrc)
      //   let Pmethod = this.$store.getters.btnpremissAll[this.permission.playerPlay].method
      //   playerPlay(Purl,Pmethod).then(res => {
      //   this.playButton = true
      //   this.multipleValue= 1
      //   // this.playerStartTime = new Date(`${this.playTimeYYR} ${this.getTime(this.playerNowTime)}`).getTime()
      //   this.playerStartTime = this.playerNowTime
      // }).catch(err => {
      //   this.$message.error('播放请求错误！', err)
      // })
      this.playerStartTime = this.playerNowTime
    ...
    multiple(n) {
      ...
      // const data = {
      //   ssrc: this.mySsrc,
      //   scale: String(this.multipleValue)
      // }
      // let Surl = this.$store.getters.btnpremissAll[this.permission.playerSpeed].url
      // let Smethod = this.$store.getters.btnpremissAll[this.permission.playerSpeed].method
      // playerSpeed(Surl,Smethod,data).then(res => {
      // }).catch(err => {
      //   this.$message.error('倍速请求错误！', err)
      // })
    ...
    jumpPlay(time) {
      ...
      if(clickTime > nowPlaySlotstart && clickTime < nowPlaySlotEnd) {
        ...
        // let Rmethod = this.$store.getters.btnpremissAll[this.permission.playerRange].method
        // this.forwardS = (clickTime - nowPlaySlotstart) / 1000
        // playerRange(Rurl.replace('{ssrc}',this.mySsrc).replace('{range}',this.forwardS),Rmethod).then(res => {
        // }).catch(err => {
        //   this.$message.error('请求错误！', err)
        // })
      }
    ...
    backAllMasks() {
      ...
      let Aurl = this.$store.getters.btnpremissAll[this.permission.getQueryAlarm].url
      let Amethod = this.$store.getters.btnpremissAll[this.permission.getQueryAlarm].method
      getQueryAlarm(Aurl,Amethod,{ deviceId: this.myId.deviceId, timeStamp: this.playTimeYYR })
      .then(res => {
        if (res.meta.status === 200) {
          this.warningSpan= res.data.data
          this.warningSpan.map(item=>{
            item.percentage = ((new Date(item.alarmTime).getHours()*3600+new Date(item.alarmTime).getMinutes()*60+new Date(item.alarmTime).getSeconds())/86400).toFixed(3)*100 + '%'
          })
          if(res.data.count > 0){
            this.warningSize = '新告警'
          }
        }
      }).catch(err => {
        this.$message.error('告警请求失败', err)
    ...
    takeVideo() {
      ...
      const data = {
        ...
        ip: this.baseMediaUrl + ':80',
        app: 'rtp',
        setTime: interval,
        ssrc: this.getSsrc16(this.mySsrc),
        validTime: this.takeVideoForm.day
      }
      let Rurl = this.$store.getters.btnpremissAll[this.permission.startRecording].url
      let Rmethod = this.$store.getters.btnpremissAll[this.permission.startRecording].method
      startRecording(Rurl,Rmethod,data).then(res => {
        if (res.meta.status === 200) {
          this.uuid = res.data
    ...
    endDown() {
      ...
      let Rurl = this.$store.getters.btnpremissAll[this.permission.endRecording].url
      let Rmethod = this.$store.getters.btnpremissAll[this.permission.endRecording].method
      endRecording(Rurl,Rmethod,data).then(res => {
        this.isTabeVideoOn = true
        this.$message.success('已结束录制')
      }).catch(err => {
    ...
    warningSearch() {
      const data = {
        ...
        pageSize:1000,
        pageNum:1,
        startTime : `${this.playTimeYYR} ${Moment(this.warningSearchForm.time[0]).format('HH:mm:ss')}`,
        endTime : `${this.playTimeYYR} ${Moment(this.warningSearchForm.time[1]).format('HH:mm:ss')}`,
        descption : this.warningSearchForm.content
      }
      let Wurl = this.$store.getters.btnpremissAll[this.permission.getSearchWarning].url
      let Wmethod = this.$store.getters.btnpremissAll[this.permission.getSearchWarning].method
      getSearchWarning(Wurl,Wmethod,data).then(res=>{
        if (res.meta.status === 200) {
          this.warningSpan= res.data.data
          this.warningSpan.map(item=>{
            item.percentage = ((new Date(item.alarmTime).getHours()*3600+new Date(item.alarmTime).getMinutes()*60+new Date(item.alarmTime).getSeconds())/86400).toFixed(3)*100 + '%'
   ...
   dialogMarksF () {
    ...
    // 删除书签
    deleteMarks(data, index) {
      let durl = this.$store.getters.btnpremissAll[this.permission.deleteRecordTag].url
      let dmethod = this.$store.getters.btnpremissAll[this.permission.deleteRecordTag].method
      deleteRecordTag(durl.replace('{id}', data.id),dmethod).then(res => {
        if (res.meta.status === 200) {
           this.searchMasks(false)
          // let Turl = this.$store.getters.btnpremissAll[this.permission.getRecordTag].url
          // let Tmethod = this.$store.getters.btnpremissAll[this.permission.getRecordTag].method
    ...
    snapPhoto1() {
      const obj = {
        ...
        interval: 0,
        count: 1,
        ssrc: this.getVideoHttpBySsrc(this.mySsrc)
      }
      let Surl = this.$store.getters.btnpremissAll[this.permission.postSnapPhoto].url
      let Smethod = this.$store.getters.btnpremissAll[this.permission.postSnapPhoto].method
      postSnapPhoto(Surl,Smethod,obj).then(res => {
        if (res.meta.status === 200) {
          this.$message.success('抓拍成功')
        }
      }).catch(err => {
        this.$message.error('抓拍失败', err)
      })
    },
