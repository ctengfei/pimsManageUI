  ...
  <div class="app-container-gis">
    <div id="cesiumContainer">
      ...
      <div class="rightBox" v-if="addFormShow">
        <div class="addTitle">{{ addOrEditTitle }}行为识别任务</div>
        ...
        <div style="margin-top: 15px">
          <el-form
            ...
            label-width="110px"
          >
            <el-form-item label="园区" prop="area">
              <el-select
            ...
            <!-- <el-form-item label="创建时间" v-if="$route.query.type == 'look'">
              <el-input
            ...
            <div class="tableBox">
              <div class="tableBoxTitle">
                <div>
                  ...
                  <span>布控摄像机</span>
                  <span style="color: #f05656; font-size: 12px" v-show="!flag"
                    >请添加布控摄像头</span
                  >
                </div>
                <el-button
              ...
              <div>
                <el-table
                  :data="addForm.intelligenceAnalysisTaskDeviceDTOs"
                  style="width: 100%"
                  height="200px"
                >
                  <el-table-column
...
import {
  ...
  detailTask,
  getTaskType,
} from "@/api/monitor/perimeterTask";
...
export default {
  ...
  data() {
    return {
      ...
      addForm: {
        area: "",
        intelligenceAnalysisTaskDeviceDTOs: [],
        description: "",
        taskName: "",
        startTime: "",
        endTime: "",
  ...
  methods: {
    onSubmit1() {
      if (this.tempList.length <= 12) {
        ...
        if (this.addOrEditType == "edit") {
          let temp = this.addForm.intelligenceAnalysisTaskDeviceDTOs.concat(
            this.tempList
          );
          let list = uniqueArray(temp, "code");
          this.addForm.intelligenceAnalysisTaskDeviceDTOs = list;
        } else {
          this.addForm.intelligenceAnalysisTaskDeviceDTOs = this.tempList;
        }
      } else {
    ...
    getTaskTypes() {
      getTaskType().then((res) => {
        this.taskTypeArr = formatData(res.data);
      });
    },
    // 获取树选中摄像头id
    ...
    getDetail(id) {
      detailTask(id).then((res) => {
        ...
        this.addForm.creatTime = [res.data.startTime, res.data.endTime];
    ...
    // 取消创建
    goBack() {
      this.$router.push({ name: "BehaviorAnalysisManage" });
    },
    // 删除摄像机
    deleteCamera(index, item) {
      this.addForm.intelligenceAnalysisTaskDeviceDTOs.splice(index, 1);
    },
    // 取消
    onCancel() {
    ...
    onSubmit() {
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.addForm.startTime = this.addForm.creatTime[0];
          this.addForm.endTime = this.addForm.creatTime[1];
          if (this.addForm.intelligenceAnalysisTaskDeviceDTOs.length) {
            if (this.addOrEditType == "add") {
              addTask(this.addForm).then((res) => {
                this.addActive = false;
                this.addForm.intelligenceAnalysisTaskDeviceDTOs = [];
                this.$refs.addForm.resetFields();
                this.$message.success("创建成功");
                this.$router.push({ name: "BehaviorAnalysisManage" });
              });
            } else {
              editTask(this.$route.query.id, this.addForm).then((res) => {
                this.addActive = false;
                this.addForm.intelligenceAnalysisTaskDeviceDTOs = [];
                this.$refs.addForm.resetFields();
