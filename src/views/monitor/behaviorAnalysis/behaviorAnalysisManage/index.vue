      ...
      <div class="homeShow" v-if="contentTable">
        <div class="flex">
          <el-form ref="formSearchs" inline :model="formSearch" v-if="showSearch">
            <el-form-item prop="taskName" label="行为识别任务">
              <el-input placeholder="请输入行为识别任务名称" v-model="formSearch.taskName"></el-input>
            </el-form-item>
          ...
          <el-row :gutter="10" class="mb8">
            ...
            <el-button type="primary" @click="onAddTask()">创建行为识别任务</el-button>
        ...
        <div style="height:calc(100% - 50px)">
            ...
            >
              ...
              <el-table-column prop="stateStr" label="任务状态" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    // table表格修改
    handleUpdate(row){
      this.$router.push({name:"BehaviorAnalysisManage/addEdit",query:{id:row.id,type:"edit"}})
    },
    // table表格启用
    handleStart(row){
    ...
    // table表格查看
    handleLook(row){
      this.$router.push({name:"BehaviorAnalysisManage/addEdit",query:{id:row.id,type:"look"}})
    },
    // 批量删除
    allDel(){
      this.$confirm("此操作将删除选中的行为识别任务信息, 是否继续?", "提示", {
    ...
    // 新增车辆布控任务
    onAddTask(){
      this.$router.push({name:"BehaviorAnalysisManage/addEdit",query:{type:"add"}})
    },
    // add新增确认
    onSubmit() {
