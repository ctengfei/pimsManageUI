<template>
  ...
  export default {
    ...
    methods: {
      // 重置
      onReset(){
        if(this.$refs['formSearchs'] !== undefined){
      ...
      // table表格修改
      handleUpdate(row){
        this.$router.push({name:"FaceControl/addEdit",query:{id:row.id,type:"edit"}})
      },
      // table表格启用
      handleStart(row){
      ...
      // table表格查看
      handleLook(row){
        this.$router.push({name:"FaceControl/addEdit",query:{id:row.id,type:"look"}})
      },
      // 批量删除
      allDel(){
      ...
      // 新增人脸布控任务
      onAddTask(){
        this.$router.push({name:"FaceControl/addEdit",query:{type:"add"}})
      },
      // 人员管理名单
      onPersonManage(){
        this.$router.push({name:"BlockList",query:{type:"target"}})
      },
      // 白名单管理
      onPersonWhite(){
        this.$router.push({name:"BlockList/personWhite",query:{type:"target"}})
      },
      // add新增确认
      onSubmit() {
