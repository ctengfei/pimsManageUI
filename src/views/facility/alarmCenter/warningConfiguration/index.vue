  ...
  <div class="app-container">
        ...
        <div class="addBox"> -->
          ...
          <el-form ref="formSeach" inline v-model="formSeach" v-if="showSearch">
            <el-form-item prop="searchKey" label="规则名称">
              <el-input placeholder="请输入规则名称" clearable v-model="formSeach.searchKey"></el-input>
            </el-form-item>
          ...
          <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" :disabled="!multipleSelection.length" plain @click="onSatrt" >批量启用</el-button>
                <el-button type="primary" :disabled="!multipleSelection.length" plain @click="onDisabled" >批量禁用</el-button>
                <el-button type="danger" :disabled="!multipleSelection.length" plain @click="onDelete" >批量删除</el-button>
                <el-button type="primary" plain @click="onAdd">新增规则</el-button>
            </el-col>
            <right-toolbar :showSearch.sync="showSearch" @queryTable="getRoundList"></right-toolbar>
          </el-row>
            <el-table
              ref="multipleTable"
              :data="roundList"
              ...
              <el-table-column
                ...
                show-overflow-tooltip>
                <template slot-scope="scope">
                  <el-button @click="editCrea(scope.row.id)" type="text" icon="el-icon-edit"> 编辑</el-button>
                  <el-button type="text" @click="delCrea(scope.row.id)" icon="el-icon-delete">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
      ...
      >
        <div class="rightBox padding-10" >
          ...
          <div style="margin-top: 15px;">
            ...
            <el-form-item label="规则名称" prop="ruleName" :rules="[{required: true, message: '请输入名称', trigger: 'blur'}]">
              <el-input v-model="cameraForm.ruleName"></el-input>
            </el-form-item>
            <el-form-item label="规则说明" prop="description">
              <el-input v-model="cameraForm.description"></el-input>
            </el-form-item>
            ...
            <el-form-item label="告警事件" prop="alarmEvent" :rules="[{required: true, message: '请选择', trigger: '[blur,change]'}]">
              <!-- <el-input v-model="cameraForm.position"></el-input> -->
              <el-select v-model="cameraForm.alarmEvent" placeholder="告警事件" clearable>
                <el-option
          ...
          <div class="tableBox">
            <div class="tableBoxTitle">
              <span>设备列表</span>
              <span class="addCarmea"  @click="addCarmea"><i class="el-icon-plus"></i>添加</span>
            </div>
            <el-table
              height="200"
          ...
          <div class="tableBox">
            <div class="tableBoxTitle">
              <span>联动规则</span>
              <span class="addCarmea"  @click="addCarmeas"><i class="el-icon-plus"></i>添加</span>
            </div>
            <el-table height="200" :data="linkageRule" style="width: 100%" >
              ...
              <el-table-column label="设备名称" show-overflow-tooltip prop="deviceName" />
              <el-table-column label="执行动作" show-overflow-tooltip prop="actionStr" width="100" />
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <div class="tableOperation">
                    <el-button
    ...
    >
        ...
        <el-form label-width="80px" :model="ruleForm" :rules="rules" ref="ruleForm">
          <el-col>
            <el-form-item label="执行动作" prop="action">
              <el-select clearable v-model="ruleForm.action" @change="ruleChange" style="width:100%">
                <el-option
      ...
      <div slot="footer" style="text-align:center;">
        <el-button type="primary" @click="carmeSubmit()"
          >确定</el-button
        >
        <el-button plain @click="carmeCancel()"
          >取消</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="addDeviceShow" width="820px" title="设备列表">
      <el-form ref="deviceSeach" inline :model="deviceSeach">
        <el-form-item prop="name">
          <el-input placeholder="请输入设备名称" clearable v-model="deviceSeach.name"></el-input>
        </el-form-item>
        ...
        <el-form-item>
          <el-button icon="el-icon-search" type="primary" @click="deviceSearch">搜索</el-button>
          <el-button  icon="el-icon-refresh-left" @click="deviceReset">重置</el-button>
        </el-form-item>
      </el-form>
      <el-table
        ref="deviceTable"
...
<script>
  ...
  import {getConfigType, getTableData, editId, add, edit, Delete, mulDelete, Start, Disabled, linkageRoleActive,getConfigEvent} from '@/api/alarmCenter/warningConfiguration';
  ...
  export default {
    ...
    data() {
      return {
        ...
        ruleAction:{deviceId:'', deviceName:'', deviceType:'', action: '', actionStr:''},
        getDeviceLists:[],
        addDeviceShow: false,
        deviceType:[
          {
    ...
    methods: {
      ...
      // 启用
      onSatrt(){
        Start(this.multipleSelection).then(res=>{
          if(res.code === 200){
      ...
      // 禁用
      onDisabled(){
        Disabled(this.multipleSelection).then(res=>{
          if(res.code === 200){
      ...
      onDelete(){
        ...
        mulDelete(this.multipleSelection).then(res=>{
          if(res.code === 200){
      ...
      // 获取计划列表
      getRoundList(){
        getTableData({pageNum:this.pageNum,pageSize:this.pageSize,searchKey:this.formSeach.searchKey,ruleStatus:this.formSeach.ruleStatus}).then(res=>{
          console.log(res)
          if(res.code === 200){
      ...
      addDeviceRule(){
        if(this.ruleAction.deviceId && this.ruleAction.action){
          ...
          this.ruleAction={deviceId:'', deviceName:'', deviceType:'', action: '', actionStr:''}
      ...
      startStatus(item){
        ...
        this.$confirm("此操作将启用当前计划, 是否继续?", "提示", {
          ...
          .then(() => {
            roundApi.startRound({activeStatus: activeStatus, planId:item.id}).then((res) => {
              if(res.code === 200){
                this.getRoundList()
                this.$message({
                  type: "success",
        ...
        this.$refs['addForm'].validate((valid) => {
          if (valid) {
            if(!this.deviceTableData.length){
              ...
              console.log(deviceIds,"this.deviceTableData.filter(item=> item.deviceId)")
              // this.cameraForm.alarmLinkageActions = this.linkageRule
              if(this.addFormType == "add"){
                add(this.cameraForm).then(res=>{
                  if(res.code === 200){
                    this.addFormShow = false;
                    this.$refs.addForm.resetFields();
                    this.getRoundList()
                ...
                edit(this.cameraForm.id,this.cameraForm).then(res=>{
                  if(res.code === 200){
                    this.addFormShow = false;
                    this.$refs.addForm.resetFields();
                    this.getRoundList()
...
::v-deeep .el-input__inner,.el-date-editor--daterange{
