<template>
  <div class="app-container-gis">
    <div id="cesiumContainer">
      ...
      ></gis-map>
      <div class="leftBox padding-10" v-if="leftBoxShow">
        <audio id="audio" src="@/assets/audio/alarm.mp3" />
        <div class="addBox">
          <div style="margin: 0 5px; width: 100%">
            <el-row :gutter="10">
              <el-form ref="alarmForm" :model="formSeach" style="width: 100%">
                <el-col :span="24">
                  <el-form-item prop="alarmCode">
                    <el-input
                      v-model="formSeach.alarmCode"
                      placeholder="请输入告警编号"
                ...
                <el-col :span="24">
                  <el-form-item prop="eventCode" label="告警事件">
                    <el-select
                      v-model="formSeach.eventCode"
                      style="width: 286px"
                      placeholder="请选择事件名称"
                      clearable
                    >
                      <el-option
                        v-for="item in eventNameCodeArr"
                        :key="item.eventCode"
                ...
                <el-col :span="12">
                  <el-form-item>
                    ...
                    <el-button icon="el-icon-refresh-left" @click="carReset()"
                      >重置
                    </el-button>
                  </el-form-item>
                </el-col>
            ...
            <div class="listWrap">
              ...
              >
                <div class="listTop" @click="nowVidoe(item)">
                  ...
                  <div class="listRight">
                    <div>
                      <span v-if="item.status == 'unprocessed'">待处理</span>
                      <span v-else-if="item.status == 'closed'">已关闭</span>
                      <span v-else-if="item.status == 'issued'">已下发</span>
                    </div>
                    <div
                      :class="[
                        'cricle',
                        item.alarmLevel == 'normal'
                          ? 'handCircle'
                          : item.alarmLevel == 'emergency'
                          ? 'redCircle'
                          : 'doneCircle',
                      ]"
                    >
      ...
      <div @click="onBackCancel" class="backtext" v-if="backShow">
      ...
      <div class="rightBox padding-10" v-if="roundPatrolShow">
        <div class="listdetailBox">
          <div class="listArr">
            <div class="listTop">
              <div class="listLeft">
                <div>
                  [{{ roundPatrol.businessName }}]{{ roundPatrol.eventName }}
                </div>
                <div>{{ roundPatrol.deviceName }}</div>
                <div>{{ roundPatrol.triggerTime }}</div>
              </div>
              <div class="listRight">
                <div>待处理</div>
                <div
                  :class="[
                    'cricle',
                    roundPatrol.alarmLevel == 'normal'
                      ? 'handCircle'
                      : roundPatrol.alarmLevel == 'emergency'
                      ? 'redCircle'
                      : 'doneCircle',
                  ]"
                >
        ...
        <div style="margin-top: 5px">
          ...
          >
            <el-form-item label="设备名称">
              <span>{{ roundPatrol.deviceName }}</span>
            </el-form-item>
        ...
        <div style="margin-top: 40px; color: #fff">视频图像</div>
        <el-divider></el-divider>
        <div
          style="line-height: 26px; color: #fff; font-size: 14px"
          v-if="roundPatrol.alarmRecords.length"
        >
          录像视频
          <el-row>
            <el-col :span="24"
              ...
              <div
                class="imgBox"
                v-for="(item, index) in roundPatrol.alarmRecords"
                :key="index"
              >
                <svg-icon
        ...
        <div
          style="line-height: 36px; color: #fff; font-size: 14px"
          v-if="roundPatrol.captureRecords.length"
        >
          关联图片
          <el-row type="flex" class="row-bg" justify="left">
            <el-col
              :span="6"
              v-for="(item, index) in roundPatrol.captureRecords"
              :key="index"
            >
              <div class="grid-content bg-purple">
                <div class="imgBox">
                  <el-image
                    ...
                    :src="item.pictureUrl"
                    :preview-src-list="pictureUrlArr"
                  >
                  </el-image>
                </div>
              </div>
    ...
    <el-dialog :visible.sync="crameShow">
      <div style="width: 48vw; height: 60vh">
...
import {
  ...
  getConfigCodeEvent,
  getstatisticsEvent,
} from "@/api/alarmCenter/warningRealtime";
...
export default {
  ...
  data() {
    return {
      ...
      businessArr: [], //业务分类
      alarmLevel: [
        { value: "NORMAL", label: "普通告警" },
        { value: "EMERGENCY", label: "严重告警" },
        { value: "VERY_EMERGENCY", label: "紧急告警" },
      ],
      alarmStatus: [
  ...
  mounted() {
    this.initmap();
    this.getConfigdataEvent();
    this.getDataStatisticsEvent();
    this.getTableData();
  },
  computed: {
    // roundList() {
  ...
  methods: {
    ...
    getConfigdataEvent() {
      getConfigCodeEvent()
        .then((res) => {
          this.eventNameCodeArr = res.data.eventNames;
        })
        .catch((err) => {
          this.$message.error(err.message);
        });
    ...
    deviceInfo(carmeaInfo) {
      ...
      if (!carmeaInfo.alarmLevel) {
        ...
        // 手动告警
        this.alarmShow = true;
        let temp = this.businessArr.filter(
          (item) => item.deviceType == carmeaInfo.deviceType
        );
    ...
    getTableData() {
      ...
      if (this.time.length) {
        this.formSeach.startTime = this.time[0];
        this.formSeach.endTime = this.time[1];
      }
      getTableData(this.formSeach)
        .then((res) => {
          if (res.code === 200) {
    ...
    // 关闭视频弹窗
    onclose() {
      this.$refs["videoPlayerGroup"].destroyAllDialog();
    ...
    roundPatrolCancel() {
      this.$refs["roundPatalForm"].validate((valid) => {
        if (valid) {
          alarmAssign({
            alarmCode: this.roundPatrol.code,
            content: this.roundPatrol.remark,
            submit: true,
          })
            .then((res) => {
              if (res.code === 200) {
    ...
    roundPatrolSubmit() {
      this.$refs["roundPatalForm"].validate((valid) => {
        if (valid) {
          alarmClose({
            alarmCode: this.roundPatrol.code,
            closeReason: this.roundPatrol.remark,
          })
            .then((res) => {
              if (res.code === 200) {
                this.roundPatrolShow = false;
