        ...
        <el-col :span="24"
          ...
          <VTable :havecellclick="false"
            ...
            <!-- label-width="auto"
                label-position="left" -->
            <template #search>
              <VTableSearchItem label="人员类型">
                <el-select @change="rytypeChange"
                           v-model="rytype"
                           placeholder="请选择人员类型">
                  <el-option v-for="dict in tsryList"
                             :key="dict.val"
                             :label="dict.label"
                             :value="dict.val"></el-option>
                </el-select>
              </VTableSearchItem>
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: '160px',
          render: (h, { row }) => {
            const val = this.dict?.type?.bq_tsrq
            ...
            return (
              <span>
                <VTableCell
                  type="查看"
                  icon="el-icon-view"
                  promiseStr='jcyw:syrk:tsrq:view'
                  onClick={() => this.onView(row)}
                />
                <VTableCell
                  type="修改"
                  icon="el-icon-edit"
                  promiseStr='jcyw:syrk:tsrq:edit'
                  onClick={() => this.onEdit(row, this.rytype)}
                />
                <VTableCell
                  type="删除"
                  icon="el-icon-delete"
                  // title={row.xm}
                  promiseStr='jcyw:syrk:tsrq:remove'
                  onClick={() => this.onDel(row, this.rytype)}
                />
              </span>
            )
          }
  ...
  methods: {
    getStatistics () {
      getStatistics().then(res => {
        this.optionObj = {
          name: '特殊人群',
          num: res.data.tsrqNum
        }
        this.sexList = res.data.ageRange

        this.renyuanList = res.data.tsrqList
      })
    },
    ...
    hjrkCountNumMethod () {
      this.religion = []
      getTotal({ type: 'bq_tsrq' }).then(async (res) => {
    ...
    onView (row) {
      // console.log(row, '传的参数')
      this.show = true
      // this.depData = row
      this.$refs.editModal.autoComplete(row)
      this.isEdit = false
    },
    onEdit (row, type) {
      ...
      this.$confirm('确定删除吗?', '系统提示', {
        ...
        .then(async () => {
          ...
          if (row.id) {
            ...
            let delLdrkRes = await multipleDelGark(this.delIds, this.delbqIds)
            if (!delLdrkRes) return
          }
          this.$message({
            message: '删除成功',
    ...
    async submitForm (form, type) {
      if (form.id) {
        if (type == 1) {
          var addRes = await updateGark({ xmsfry: form }, type)
        } else if (type == 2) {
          var addRes = await updateGark({ sqjzry: form }, type)
        } else if (type == 3) {
          var addRes = await updateGark({ jszahz: form }, type)
        } else if (type == 4) {
          var addRes = await updateGark({ xdry: form }, type)
        } else if (type == 5) {
          var addRes = await updateGark({ azbwxry: form }, type)
        }
        if (!addRes) return
        this.$modal.msgSuccess('修改成功')
      } else {
        if (type == 1) {
          var addRes = await addGark({ xmsfry: form }, type)
        } else if (type == 2) {
          var addRes = await addGark({ sqjzry: form }, type)
        } else if (type == 3) {
          var addRes = await addGark({ jszahz: form }, type)
        } else if (type == 4) {
          var addRes = await addGark({ xdry: form }, type)
        } else if (type == 5) {
          var addRes = await addGark({ azbwxry: form }, type)
        }
        if (!addRes) return
