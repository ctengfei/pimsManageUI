      ...
      <div class="mainBox">
        ...
        <div ref="ds"
          ...
          <div class="gy_box dsDaBox">
            ...
            <div class="gy_margin dsBox stateShou"
          ...
          <div style="display: flex;">
            <div class="gy_box spDaBox">
              ...
              <div style="box-sizing: border-box; padding-top: 10px">
                <div class="shangpu_box spDanBox stateShou"
                     :style="spStyle"
        ...
        <div style="display: flex;"
          ...
          <div class="gy_jsjl gy_DXbox dxDaBox">
            ...
            <div class="gy_margin dxBox stateShou"
...
export default {
  ...
  methods: {
    ...
    getWidth () {
      ...
      this.$nextTick(() => {
        ...
        if (this.dsList.length) {
          ...
          dsDaBoxNum = (dsDaBoxStyle.substring(0, dsDaBoxStyle.length - 2) * 1).toFixed(0)
        }
        if (this.spList.length) {
          let spDaBoxStyle = window.getComputedStyle(spDaBox, null).width;
          spDaBoxNum = (spDaBoxStyle.substring(0, spDaBoxStyle.length - 2) * 1).toFixed(0)
        }
        if (this.dxList.length) {
          let dxDaBoxStyle = window.getComputedStyle(dxDaBox, null).width;
          dxDaBoxNum = (dxDaBoxStyle.substring(0, dxDaBoxStyle.length - 2) * 1).toFixed(0)
        }
        ...
        for (let c = 1; c <= that.newObj.dslcs; c++) {
          ...
          for (let h = 1; h <= that.newObj.dsdchs; h++) {
            ...
            let huObj = {
              ...
              filed2: `${that.floorObj.name + d * 1}单元${(c * 1 + that.newObj.splcs) * 100 + h * 1}室`,
              filed3: 63,
    ...
    getErCiDsList () {
      ...
      let spLastNum = this.spList[0].children[0].label * 1
      this.dsList.forEach(dan => {
        ...
        dan.children.forEach((ceng, cengIndex) => {
          let cengObj = {
            filed1: `${(spLastNum) * 1 + cengIndex + 1}层`,
            filed2: `${danDizhi}${(spLastNum) * 1 + cengIndex + 1}层`,
            filed3: ceng.filed3,
            filed4: `${(spLastNum) * 1 + cengIndex + 1}`,
            filed6: ceng.filed6,
            filed7: '',
            label: (spLastNum) * 1 + cengIndex + 1,
            id: ceng.id,
      ...
      return new Promise((relove, reject) => {
        ...
        if (val == 0) {
          if (state == 'ds') {
            ...
            // this.delList = [...this.delList, ...this.spList[0].id]
            this.delList.push(this.spList[0].id)
            this.spList = []
          }
        }
    ...
    setSpCengList (val, oldVal) {
      ...
      for (let c = num; c <= that.newObj.splcs; c++) {
        ...
        for (let h = 1; h <= that.newObj.spdchs; h++) {
          ...
          let huObj = {
            filed1: `${c * 100 + h * 1}室`,
            filed2: `${that.floorObj.name}${c * 100 + h * 1}室`,
            filed3: 63,
            filed4: `${cengObj.label}`,
            filed6: '室',
            filed7: houzhui,
        ...
        for (let c = 1; c <= that.newObj.dxlcs; c++) {
          ...
          for (let h = 1; h <= that.newObj.dxdchs; h++) {
            ...
            let huObj = {
              ...
              filed2: that.floorObj.name + 'B' + d + '单元' + 'B' + (c * 100 + h * 1) + '室',
              filed3: 63,
