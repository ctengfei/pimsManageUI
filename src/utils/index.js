...
// 根据路径下载文件
export function downloadPlugin(name, suffix) {
  var url = process.env.VUE_APP_UNIT + '/videomonitorSetupX86v1.0.0.exe'
...
export function formatPhone(phone) {
  if (phone != null) {
    const newNum = phone.replace(/(\d{3})\d*(\d{4})/, '$1****$2')
    return newNum
  }
  return null;
}
...
export function formatIdCard(idCard) {
  if (idCard != null) {
    const newNum = idCard.replace(/^(.{4})(?:\d+)(.{2})$/, "\$1************\$2")
    return newNum
  }
  return null;
}
...
// 返回数据转换为key：value格式
export function formatData(arrObj){
  let temp = [];
  for(var i in arrObj){
...
/*
 * json数组去重
 * @param: {Array} array 去重之前的数组
 * @param  {String} key  需要去重的字段值
